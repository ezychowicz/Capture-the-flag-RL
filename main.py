...
def main():
    ...
    # training
    maxG = -200
    for typeOfMap in [cannons_apples_generator.CannonsApples]: #peaceful_generator.PeacefulGenerator,singular_cannon_generator.SingularCannon, multiple_cannons_generator.MultipleCannons, 
        for game_map in game_map_generator(typeOfMap, 5):
            ...
            for episode in range (100):
                print(f"EPISODE = {episode}, MAP DIFFICULTY = {typeOfMap.__name__}")
                G = epoch.runEpisode()
                print(f"RETURN = {G}")
                maxG = max(maxG, G)
                simulation.cnt = 0
